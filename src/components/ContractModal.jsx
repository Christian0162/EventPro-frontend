            ...
            if (result.isConfirmed) {
                try {
                    ...
                    await addDoc(collection(db, "notifications"), {
                        ...
                        message: `The supplier "${supplierData?.supplier_name}" has approved the contract with ID: ${contract[0].id}.`,
        ...
        if (result.isConfirmed) {
            try {
                ...
                await addDoc(collection(db, "notifications"), {
                    ...
                    message: `The Event ${eventData?.event_name} has been completed Contract ID: "${contract[0].id}". Your balance will be updated accordingly.`,
                    createdAt: serverTimestamp(),
                                ...
                                {contract.map((cont, index) => {
                                    ...
                                    return (
                                        <div key={index}>
                                            ...
                                            <div className="flex justify-between bg-gradient-to-r from-blue-600 to-blue-800 rounded-t-xl mb-2 px-7 py-7 gap-2">
                                                ...
                                                <div className='flex flex-col gap-2 text-sm items-center'>
                                                    <div className='flex items-center gap-2  px-12'>
                                                        ...
                                                        <span className={`block text-xs text-white px-3 ${cont.status === "Pending" ? "bg-yellow-600" : "bg-green-500"} py-1 rounded-full text-sm`}>{cont.status}</span>
                                                    </div>
                                                </div>
                                            ...
                                            {userData?.role != "Supplier" && contract[0].status !== "Completed" && (
                                                <>
                                                    <div className='flex mb-2'>
                                                        ...
                                                        {cont.status === "Approved" && (total_paid - total_fees) !== service_price && (
                                                            <button
                                                        ...
                                                        {event_id === user_id && cont.status === "Pending" && (
