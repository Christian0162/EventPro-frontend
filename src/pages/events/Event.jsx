            ...
            {userEvents.length > 0 && (
                <div className="grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 grid gap-5">
                    {userEvents.map((events, index) => (
                        ...
                        >
                            {/* event cards */}
                            <div className="group flex flex-col justify-between transition-all duration-200 h-full w-full border-1 bg-white border-gray-200 hover:shadow-2xl hover:-translate-y-3 p-6 rounded-lg mt-6 ">
                                <div>
                                ...
                                {userData.role === "Supplier" && (
                                    <>
                                        ...
                                        {!gettingShop && supplierData?.supplier_name?.length > 0 && (
                                            <button onClick={() => handleApply(events.id, events.user_id)} disabled={applications.find(app => app.event_id === events.id)?.status === "Pending" || applications.find(app => app.event_id === events.id)?.status === "Approved"} className={`block text-center py-3 w-full ${applications.find(app => app.event_id === events.id)?.status === "Pending" || applications.find(app => app.event_id === events.id)?.status === "Approved" ? 'bg-blue-300' : 'bg-blue-600 hover:bg-blue-700'} text-white font-bold rounded-lg`}>
                                        ...
                                        {!gettingShop && !supplierData.supplier_name?.length > 0 && (
                                            <span className="block text-center bg-gray-200 py-2 rounded-md text-gray-600">
                                                Need shop to apply
                                            </span>
                                        )}
                                    </>
